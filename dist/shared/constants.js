...
const CONSTANTS = (() => {
    ...
    function EXTRAPOLATE_PLAYER_POSITION(data, deltaTime) {
        // Don't mutate data
        data = JSON.parse(JSON.stringify(data));
    ...
    function GET_PLAYER_POSITION_AFTER_WALL_COLLISION(oldX, oldY, playerX, playerY, segments) {
        return segments
            .map(segment => {
            const [x, y] = postCollisionPosition([playerX, playerY], segment);
            return [[x, y], distance(playerX, playerY, x, y)];
        })
            .filter(([_, dist]) => dist > 0)
            .map(([[x, y]]) => [x - playerX, y - playerY])
            .reduce(([px, py], [dx, dy]) => [px + dx, py + dy], [playerX, playerY]);
        function postCollisionPosition(player, wall) {
            ...
            if (Math.abs(sx1 - sx2) < EPSILON) // Handle the case of the vertical line:
             {
                const collides = Math.min(sy1, sy2) <= y && y <= Math.max(sy1, sy2) &&
                    Math.min(oldX, x - pr) < sx1 && sx1 < Math.max(oldX, x + pr);
                return collides
                    ? [sx1 + Math.sign(oldX - sx1) * pr, y]
                    : tip_or_nothing();
            }
            ...
            const collides = Math.min(sx1, sx2) <= x0 && x0 <= Math.max(sx1, sx2) &&
    ...
    function LINE_SEGMENT_INTERSECTION_POINT(l1, l2) {
        const EPSILON = 1e-9;
        for (const p1 of l1) {
            for (const p2 of l2) {
...
var WallType;
(function (WallType) {
    WallType[WallType["BRICK"] = 0] = "BRICK";
    WallType[WallType["FENCE"] = 1] = "FENCE";
    WallType[WallType["NON_NEWTONIAN"] = 2] = "NON_NEWTONIAN";
})(WallType || (WallType = {}));
