...
export class Walls {
    ...
    generateBeautifulSymmetricWalls(n) {
        const minLength = 0.1;
        const maxLength = 0.4;
        const slopeCount = 4;
        ...
        const walls = [...Array(n)].map(_ => {
            ...
            const p2 = [clamp(0, x + Math.cos(rndAngle) * length, 1),
            ...
            .map(([[x1, y1], [x2, y2]]) => [[{ x: x1, y: y1 }, { x: x2, y: y2 }] // Identity
            ,
    ...
    generateRandomMap(nWalls) {
        const COMPLETELY_ESSENTIAL_BOUNDARY_WALLS = [[[0, 0], [0, 1]], [[0, 1], [1, 1]], [[1, 1], [1, 0]], [[1, 0], [0, 0]]]
        ...
        function isTraversableEverywhere(width, height, walls) {
            ...
            const round = (x) => Math.round(x * BIGNUMBER) / BIGNUMBER;
            ...
            for (let i = 0; i < allTheWalls.length; i++) {
                ...
                for (let j = 0; j < allTheWalls.length; j++) {
                    if (i === j)
                        continue;
                    const w1 = allTheWalls[i];
                    const w2 = allTheWalls[j];
            ...
            return true;
            function inExclusiveRange(x, y) {
                return 0 < x && x < width && 0 < y && y < height;
            }
            function atleastOneSlopeDiffers(path, m, x1, x2) {
...
function pseudoIntersection(_l1, _l2) {
    for (const p1 of _l1) {
        for (const p2 of _l2) {
