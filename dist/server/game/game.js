...
export class Game {
    ...
    setPlayerLag(username) {
        return (lag) => {
            this.getPlayerByName[username].data.latency = lag;
        };
    }
    naive_moveObjects(timeDelta, now) {
    ...
    moveObjects(timeDelta, now) {
        const bulletQT = new QuadTree(0, 0, 1, 1, 4);
        const collisionArgs = {};
        this.bullets = this.bullets.filter(bullet => {
            const bx = bullet.data.x;
            const by = bullet.data.y;
...
function makeCollisionFunc(bx, by, newbx, newby) {
